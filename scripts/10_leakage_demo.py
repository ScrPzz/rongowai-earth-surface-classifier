...
def main() -> None:
    ...
    df = load_samples(cfg.samples_dir, groups=DEFAULT_GROUPS, splits=[SPLIT_TRAIN], flight_table=fl, include_analysis=False)
    ...
    for k in np.unique(folds):
        te = folds == k
        m = make_xgb(device=args.device, n_estimators=args.n_estimators, seed=cfg.split.seed).fit(X[~te], y[~te])
        p = m.predict_proba(X[te])[:, 1]
        rows.append({"split": "by flight", "fold": int(k), "roc_auc": roc_auc_score(y[te], p), "avg_precision": average_precision_score(y[te], p)})
    ...
    for k, (tr, te) in enumerate(skf.split(X, y)):
        m = make_xgb(device=args.device, n_estimators=args.n_estimators, seed=cfg.split.seed).fit(X[tr], y[tr])
        p = m.predict_proba(X[te])[:, 1]
        rows.append({"split": "random rows", "fold": k, "roc_auc": roc_auc_score(y[te], p), "avg_precision": average_precision_score(y[te], p)})
    ...
    save_json({"n_rows": int(len(df)), "summary": {s: {"roc_auc": float(g["roc_auc"].mean()), "avg_precision": float(g["avg_precision"].mean())} for s, g in res.groupby("split")}}, cfg.results_dir / "leakage_demo.json")
